...
namespace AinashiLibraryCSharp.Extensions
{
    public static partial class IListExtensions
    {
        ...
        /// </summary>
        /// <typeparam name="SrcType">コピー元配列の変数型。
        /// <typeparamref name="DstType"/>との互換性がある必要があります。</typeparam>
        /// <typeparam name="DstType">コピー先配列の変数型。</typeparam>
        /// <param name="srcArray">データを格納しているコピー元の配列。
        /// インデックスによるランダムな読み取りアクセスが条件となります。</param>
        /// <param name="dstArray">コピー先の配列。</param>
        /// <param name="length">コピーする要素の数。</param>
        public static void CopyTo<SrcType, DstType>(this IList<SrcType> srcArray, IList<DstType> dstArray, int length)
        ...
        /// <param name="length">コピーする要素の数。</param>
        public static void CopyTo<SrcType, DstType>(this IList<SrcType> srcArray, int srcIdx, IList<DstType> dstArray, int dstIdx, int length)
            where SrcType : DstType
        {

            if (!dstArray.IsReadOnly)
                throw new NotSupportedException("コピー先配列は書き込みをサポートしていません。");
